...
;(async () => {
  try {
    ...
    const movieCollection = mflix.collection("movies")
    ...
    const cursor = await movieCollection.find(predicate, projection).toArray()
    ...
    const { modifiedCount } = await movieCollection.bulkWrite(moviesToMigrate)
